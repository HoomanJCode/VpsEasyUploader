...
if [ "${1:-}" = "--service" ]; then
    ...
    if [ -n "$TUSD_HOOK_SECRET_VAL" ]; then
        TUSD_CMD_LINE="$TUSD_BIN_PATH -host=0.0.0.0 -port=1080 -upload-dir=$TUSD_DATA_DIR -hooks-http=http://127.0.0.1:$PORT/tus-hook?secret=$TUSD_HOOK_SECRET_VAL -hooks-enabled-events=post-finish"
    else
...
# NOTE: tusd v2.x uses single-dash flags: -host, -port, -upload-dir, etc.    if [ -n "$TUSD_HOOK_SECRET" ]; then
        "$TUSD_BIN" \
            -host="0.0.0.0" \
            -port="$TUSD_PORT" \
            -upload-dir="$TUSD_DATA" \
            -hooks-http="$TUSD_HOOK_URL?secret=$TUSD_HOOK_SECRET" \
